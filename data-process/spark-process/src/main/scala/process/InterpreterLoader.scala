...
import process.Configs.ENTRANCE_SCRIPT
...
object InterpreterLoader {
    ...
    val ONE_MINUTE_IN_MS: Long = TimeUnit.MILLISECONDS.convert(1, TimeUnit.MINUTES)
    ...
    val outputDir: File = {
        val outputDirPath: String = FileUtils.getTempDirectory.getAbsolutePath + "/spark-records/" + DateFormatUtils.format(new Date(), "yyyy-MM-dd_hh-mm-ss")
        log.info(s"going to create outputDir:\n\t$outputDirPath")
        val _outputDir: File = FileUtils.getFile(outputDirPath)
        if (_outputDir.mkdir()) {
            _outputDir
        } else {
            log.error(s"failed to create outputDir at \n\t$outputDirPath")
            null
        }
    }
    ...
    def execTask(): Unit = {
        try {
            lastResult = ScriptHelper.processTaskScript[Any](spark, ENTRANCE_SCRIPT, lastResult)
        } catch {
            case e: Throwable =>
                log.error("failed to records records.ScriptSrc", e)
                lastResult = e
                throw e
        } finally {
