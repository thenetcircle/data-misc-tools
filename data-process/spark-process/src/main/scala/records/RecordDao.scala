...
object RecordDao {
    ...
    def save(rec: ExecutionRecord): Unit = {
        stupidTemplate((pm) => pm.makePersistent(rec))
    ...
    def cleanUp(): Unit = {
        log.info("closing up")
        Option(pm).foreach(_.close())
        Option(pmf).foreach(_.close())
        log.info("closed")
    }
    ...
    def getLastExecution(path: String): ExecutionRecord = {
        val q: Query = pm.newQuery("JPQL", s"select er from records.ExecutionRecord er where er.scriptSrc.path='$path' order by er.startedAt desc limit 1")
