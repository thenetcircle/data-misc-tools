...
public class KafkaHelper {
    public static final MapObjectInspector KAFKA_CFG_INSP = getStandardMapObjectInspector(
        javaStringObjectInspector,
        javaStringObjectInspector);

    public static final StandardStructObjectInspector KAFKA_RECORD_INSP = getStandardStructObjectInspector(
    ...
    public static final StandardListObjectInspector KAFKA_RECORD_LIST_INSP = getStandardListObjectInspector(
        KAFKA_RECORD_INSP
    );

    public static final StandardListObjectInspector KAFKA_TOPIC_PARTITION_INSP = getStandardListObjectInspector(
...
//        DEFAULT_CONSUMER_CONFIGS.put(ConsumerConfig.CLIENT_ID_CONFIG, "hive-udf");
    ...
    public static Future<RecordMetadata> pushWithOutTransaction(KafkaProducer<String, String> kp,
                                                                String topic,
                                                                String key,
                                                                String value) {
    ...
    public static List<Future<RecordMetadata>> pushWithOutTransaction(KafkaProducer<String, String> kp,
                                                                      String topic,
    ...
    public static List<Future<RecordMetadata>> pushWithTransaction(KafkaProducer<String, String> kp,
                                                                   String topic,
                                                                   LinkedHashMap<String, String> records) {
        List<Future<RecordMetadata>> futureList = new ArrayList<>();
        try {
    ...
    public static List<ConsumerRecord<String, String>> poll(KafkaConsumer<String, String> kc,
        ...
        validTopics.stream().forEach(topic -> {
            ...
            parts.forEach(part -> topicAndTimesStart.put(new TopicPartition(part.topic(), part.partition()), startTime));
        });
        ...
        validTopics.stream().forEach(topic -> {
            List<PartitionInfo> parts = topicAndParts.get(topic);
            parts.forEach(part -> topicAndTimesEnd.put(new TopicPartition(part.topic(), part.partition()), endTime));
        });
    ...
    public static List<ConsumerRecord<String, String>> poll(Map consumerCfgs,
                                                            long timeout,
                                                            Long startTime,
                                                            Long endTime,
                                                            String... topics) {
        if (MapUtils.isEmpty(consumerCfgs)) return Collections.emptyList();

        Properties cfgs = mapToProperties(consumerCfgs);

        List<ImmutablePair<TopicPartition, Range<Long>>> tpAndRangeList = null;
        try (KafkaConsumer<String, String> kc = new KafkaConsumer<String, String>(mapToProperties(cfgs))) {
            ...
            List<TopicPartition> tpList = validTopics.stream()
                .map(topicAndParts::get)
                .flatMap(List::stream)
                .map(part -> new TopicPartition(part.topic(), part.partition()))
                .collect(Collectors.toList());
            Map<TopicPartition, Long> topicAndTimesStart = tpList.stream().collect(Collectors.toMap(tp -> tp, (part) -> startTime));
            Map<TopicPartition, OffsetAndTimestamp> topicPartAndOffsetNTimesStart = kc.offsetsForTimes(topicAndTimesStart);

            Map<TopicPartition, Long> topicAndTimesEnd = tpList.stream().collect(Collectors.toMap(tp -> tp, (part) -> endTime));
            Map<TopicPartition, OffsetAndTimestamp> topicPartAndOffsetNTimesEnd = kc.offsetsForTimes(topicAndTimesEnd);

            kc.assign(tpList);
            Map<TopicPartition, Long> topicPartAndOldests = kc.beginningOffsets(tpList);
            Map<TopicPartition, Long> topicPartAndLatests = kc.endOffsets(tpList);

            tpAndRangeList = tpList.stream().map(tp -> {
                Long fromInclusive = Optional.ofNullable(topicPartAndOffsetNTimesStart.get(tp))
                    .map(OffsetAndTimestamp::offset)
                    .orElse(topicPartAndOldests.get(tp));
                ...
                return ImmutablePair.of(tp, Range.between(
                    defaultIfNull(fromInclusive, Long.valueOf(0)),
                    defaultIfNull(toInclusive, Long.valueOf(0))));
            }).collect(Collectors.toList());
        }

        return tpAndRangeList.parallelStream()
            .map(pair -> poll(cfgs, timeout, pair.left, pair.right.getMinimum(), pair.right.getMaximum()))
            .flatMap(List::stream)
            .filter(rec -> rec.timestamp() > startTime && rec.timestamp() <= endTime)
            .collect(Collectors.toList());
    }

    static List<ConsumerRecord<String, String>> poll(Properties cfgs,
                                                     long timeout,
                                                     TopicPartition tp,
                                                     long start, long end) {
        List<ConsumerRecord<String, String>> reList = new ArrayList<>((int) (end - start));
        try (KafkaConsumer<String, String> kc = new KafkaConsumer<String, String>(cfgs)) {
            kc.assign(Arrays.asList(tp));
            kc.seek(tp, start);
            for (long i = start, j = end - 1; i < j; ) {
                ConsumerRecords<String, String> recs = kc.poll(timeout);
                reList.addAll(recs.records(tp));
                i += recs.count();
            }
